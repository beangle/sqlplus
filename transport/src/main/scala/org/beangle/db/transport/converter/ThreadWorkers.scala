...
object ThreadWorkers {

  def work[T](payloads: Iterable[T], job: T => Unit, threadPoolSize: Int, name: String = null): Unit = {
    val loads = new LinkedBlockingQueue[T]
    import scala.jdk.CollectionConverters.*
    loads.addAll(payloads.toSeq.asJava)
    ThreadTasks.start(new Work(loads, job), threadPoolSize, name)
  }
